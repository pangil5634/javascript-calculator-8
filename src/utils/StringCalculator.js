...
export class StringCalculator {
  ...
  parse(input) {
    ...
    if (customMatch) {
      ...
      // 다중 특수문자 이스케이프 처리
      const escapedCustomDelimiter = customDelimiter.replace(
        /[.*+?^${}()|[\]\\]/g,
        '\\$&',
      );
      ...
      if (!parsingString.includes(customDelimiter)) {
        // 커스텀 구분자가 없는 경우, 기본 구분자로 파싱 문자열 분리
        numbers = parsingString.split(/,|:/);
      } else {
        // 커스텀 구분자가 있는 경우, 커스텀 구분자로 파싱 문자열 분리
        const pattern = delimiters.join('|');
        numbers = parsingString.split(new RegExp(pattern));
      }
    } else {
      // 2) 기본 구분자인 경우
